...
public interface InCartRepository extends JpaRepository<InCart, Long> {
    ...
    @Query("select new com.example.appbackend.dto.InCartDTO(pr.name, pr.price, pr.category.name, pr.shop.name, ic.type, ic.quantity) from in_cart ic, product pr where ic.product.id = pr.id and ic.appUser.id = ?1")
