...
const NavBar = ({ items }) => {
    ...
    return (
        <div className="shadow-third mb-[1rem]">
            <div className="flex flex-col md:flex-row items-center justify-center p-3 md:px-0">
                <div className="flex items-center flex-1">
                    <div style={{ width: "100%", height: "5em", padding: "10px", }}>
                    ...
                    <div className={`flex items-center flex-shrink-0 p-1 ml-3 border min-w-fit rounded-md`}>
                        <input type="text" name="searchBar" className="transition-all focus:w-full" placeholder="Searching something"
                               onFocus={(e) => {
                ...
                <div className="flex-1 mt-4 md:mt-0">
                    <h1 className="text-lg font-semibold text-center select-none">Sneaker Market</h1>
                </div>
                <div className="flex items-end justify-end flex-1 p-3 mr-8 gap-x-6 mt-4 md:mt-0">
                    <div><span className="select-none font-extralight">{name}</span></div>
                    {localStorage.role === "USER" && (
                        <Link to="/cart">
                            ...
                            <span className="relative inline-flex rounded-full h-5 w-5 bg-sky-500 items-center justify-center text-white text-[9px]">{cartQuantity}</span>
                        </span>
                            </div>
                        </Link>)}
            ...
            <div className={menuClass} style={{ zIndex: "9" }}>
                <div className="menu-list">{items.map((item, index) => (<MenuItemsBuilder key={index} item={item.name} toPage={item.page} />))}</div>
            </div>
        </div>
    );
